import { getDefaultFlagDataUrl } from './defaultFlagAssets';
import { getSideIconSlot } from './iconConfig';
import { HistoricalEvent, IconConfig, Side } from './types';

export const parseDateValue = (dateStr: string): number => {
  if (!dateStr) {
    return 0;
  }

  const yearMonthMatch = dateStr.match(/(\d{4})[./-](\d{1,2})/);
  if (yearMonthMatch) {
    return parseInt(yearMonthMatch[1], 10) * 100 + parseInt(yearMonthMatch[2], 10);
  }

  const yearMatch = dateStr.match(/(\d{4})/);
  if (yearMatch) {
    return parseInt(yearMatch[1], 10) * 100;
  }

  return 0;
};

export const sortEventsByDate = (events: HistoricalEvent[]) => {
  return [...events].sort((leftEvent, rightEvent) => {
    return parseDateValue(leftEvent.dateStr) - parseDateValue(rightEvent.dateStr);
  });
};

const escapeXml = (text: string) =>
  text
    .replaceAll('&', '&amp;')
    .replaceAll('<', '&lt;')
    .replaceAll('>', '&gt;')
    .replaceAll('"', '&quot;')
    .replaceAll("'", '&#39;');

const wrapText = (text: string, maxCharsPerLine: number) => {
  const lines: string[] = [];
  let currentLine = '';
  let count = 0;

  for (let index = 0; index < text.length; index += 1) {
    currentLine += text[index];
    count += text.charCodeAt(index) > 255 ? 2 : 1;

    if (count > maxCharsPerLine) {
      lines.push(currentLine);
      currentLine = '';
      count = 0;
    }
  }

  if (currentLine) {
    lines.push(currentLine);
  }

  return lines.slice(0, 5);
};

const getSvgFlagMarkup = (side: Side, iconConfig?: IconConfig) => {
  const override = iconConfig?.[getSideIconSlot(side)];

  if (override?.mode === 'image' && override.value) {
    return `<image href="${escapeXml(override.value)}" width="40" height="26" preserveAspectRatio="xMidYMid slice" />`;
  }

  if (override?.mode === 'text' && override.value) {
    return `
      <rect width="40" height="26" rx="4" fill="#2a0a0a" opacity="0.92" />
      <text x="20" y="17" text-anchor="middle" fill="#ffffff" font-family="sans-serif" font-size="10" font-weight="bold">${escapeXml(override.value)}</text>
    `;
  }

  const defaultFlagDataUrl = getDefaultFlagDataUrl(side);
  if (defaultFlagDataUrl) {
    return `<image href="${escapeXml(defaultFlagDataUrl)}" width="40" height="26" preserveAspectRatio="xMidYMid slice" />`;
  }

  if (side === Side.JOINT) {
    return `
      <defs>
        <linearGradient id="jointGradExport" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stop-color="#DE2910" />
          <stop offset="100%" stop-color="#000095" />
        </linearGradient>
      </defs>
      <rect width="40" height="26" fill="url(#jointGradExport)" />
      <text x="20" y="17" text-anchor="middle" fill="white" font-family="sans-serif" font-size="8" font-weight="bold">合作</text>
    `;
  }

  if (side === Side.JAPAN) {
    return `
      <rect width="40" height="26" fill="#ffffff" />
      <circle cx="20" cy="13" r="8" fill="#bd0029" />
    `;
  }

  return `
    <rect width="40" height="26" fill="#444444" />
    <text x="20" y="17" text-anchor="middle" fill="white" font-family="sans-serif" font-size="8" font-weight="bold">其他</text>
  `;
};

const getMajorMarkerMarkup = (cardWidth: number, cardHeight: number, iconConfig?: IconConfig) => {
  const override = iconConfig?.markerMajor;
  const x = cardWidth - 30;
  const y = cardHeight - 15;

  if (override?.mode === 'image' && override.value) {
    return `<image href="${escapeXml(override.value)}" x="${cardWidth - 38}" y="${cardHeight - 38}" width="26" height="26" preserveAspectRatio="xMidYMid contain" />`;
  }

  if (override?.mode === 'text' && override.value) {
    return `<text x="${x}" y="${y}" font-size="24" fill="#FFD700">${escapeXml(override.value)}</text>`;
  }

  return `<text x="${x}" y="${y}" font-size="24" fill="#FFD700">★</text>`;
};

export const generateTimelineSVG = (events: HistoricalEvent[], iconConfig?: IconConfig) => {
  const cardWidth = 260;
  const cardHeight = 200;
  const gap = 40;
  const startX = 100;
  const totalWidth = startX + events.length * (cardWidth + gap) + 100;
  const totalHeight = 800;
  const centerY = totalHeight / 2;

  let svgContent = `
    <svg xmlns="http://www.w3.org/2000/svg" width="${totalWidth}" height="${totalHeight}" viewBox="0 0 ${totalWidth} ${totalHeight}">
      <defs>
        <linearGradient id="bgGrad" x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" stop-color="#1a0505" />
          <stop offset="50%" stop-color="#2a0a0a" />
          <stop offset="100%" stop-color="#1a0505" />
        </linearGradient>
        <filter id="shadow">
          <feDropShadow dx="2" dy="2" stdDeviation="3" flood-opacity="0.3"/>
        </filter>
      </defs>

      <rect width="100%" height="100%" fill="url(#bgGrad)" />

      <text x="50" y="80" fill="#FFD700" font-family="serif" font-size="48" font-weight="bold">中国革命战争史诗 (1931-1953)</text>
      <text x="50" y="120" fill="#ccaaaa" font-family="serif" font-size="24">The Epic History of Chinese Revolutionary Wars</text>

      <line x1="0" y1="${centerY}" x2="${totalWidth}" y2="${centerY}" stroke="#FFD700" stroke-width="4" opacity="0.8" />
  `;

  events.forEach((event, index) => {
    const isTop = index % 2 === 0;
    const x = startX + index * (cardWidth + gap);
    const y = isTop ? centerY - 50 - cardHeight : centerY + 50;
    const lineStartY = centerY;
    const lineEndY = isTop ? y + cardHeight : y;

    let titleColor = '#333333';
    let borderColor = '#888888';
    let dotFill = '#666666';

    if (event.side === Side.CCP) {
      titleColor = '#b91c1c';
      borderColor = '#ef4444';
      dotFill = '#DE2910';
    } else if (event.side === Side.KMT) {
      titleColor = '#1e40af';
      borderColor = '#3b82f6';
      dotFill = '#000095';
    }

    svgContent += `<line x1="${x + cardWidth / 2}" y1="${lineStartY}" x2="${x + cardWidth / 2}" y2="${lineEndY}" stroke="${borderColor}" stroke-width="2" />`;
    svgContent += `<circle cx="${x + cardWidth / 2}" cy="${centerY}" r="6" fill="${dotFill}" stroke="white" stroke-width="2" />`;
    svgContent += `<g transform="translate(${x}, ${y})">`;
    svgContent += `<rect width="${cardWidth}" height="${cardHeight}" rx="4" fill="#fdfbf7" stroke="${borderColor}" stroke-width="${event.isMajor ? 4 : 1}" filter="url(#shadow)" />`;
    svgContent += `<g transform="translate(${cardWidth - 45}, -10)">${getSvgFlagMarkup(event.side, iconConfig)}</g>`;
    svgContent += `<text x="15" y="30" font-family="monospace" font-weight="bold" font-size="14" fill="#666666">${escapeXml(event.dateStr)}</text>`;
    svgContent += `<text x="15" y="60" font-family="serif" font-weight="bold" font-size="20" fill="${titleColor}">${escapeXml(event.title)}</text>`;

    const lines = wrapText(event.description, 34);
    lines.forEach((line, lineIndex) => {
      svgContent += `<text x="15" y="${90 + lineIndex * 18}" font-family="serif" font-size="12" fill="#333333">${escapeXml(line)}</text>`;
    });

    if (event.isMajor) {
      svgContent += getMajorMarkerMarkup(cardWidth, cardHeight, iconConfig);
    }

    svgContent += `</g>`;
  });

  svgContent += `</svg>`;
  return svgContent;
};
